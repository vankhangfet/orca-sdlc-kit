...
// ---------------------------------------------------------------------------
// E27 — parallel REVIEW group + onFailGoto: Security Review fails once inside
...
// ---------------------------------------------------------------------------
scenario("E27 parallel-review fail-retry (group re-runs both reviews after fix)", async () => {
  const r = await runFlow({ name: "e27", config: "../test/configs/parallel-review.config.json",
    scenario: "parallel-review-fail-once.cjs", budgetMs: 90000 });
  ok("E27 not hung", !r.hung);
  eq("E27 exit code", r.code, 0);
  ...
  ok("E27 both reviews launched before the group's first wait", starts[2] > -1 && checkAfterFirstReview > starts[2]);
  ok("E27 fail jump logged", /\[fail\] Security Review FAILED( \(.*\))? -> back to "Coder" \(attempt 1\/1\)/.test(r.out + r.err));
  eq("E27 one task-create per step (cached tasks reopened, not re-created)", r.by("orchestration task-create").length, 4);
  eq("E27 worker-start total (coder x2, each review x2, testing x1)", r.by("orchestration worker-start").length, 7);
  const reopens = r.by("orchestration task-update").filter((c) => c.flags.status === "ready");
  ok("E27 coder reopened with fix note", reopens.some((c) => String(c.flags.result ?? "").includes("fix from security-review")));
  eq("E27 security-review flagged parallel", r.status?.steps.find((s) => s.id === "security-review")?.parallel, true);
  eq("E27 coder attempt 2", r.status?.steps.find((s) => s.id === "coder")?.attempt, 2);
  eq("E27 security-review attempt 2", r.status?.steps.find((s) => s.id === "security-review")?.attempt, 2);
  ...
  ok("E27 testing tasked only after the retry round settles", joinCreate > -1 && checkAfterRetry > -1 && joinCreate > checkAfterRetry);
  ok("E27 testing ok line (join after both reviews)", /\[ok\] Testing done -> \.orca\/artifacts\/TEST_REPORT\.md/.test(r.out + r.err));
  eq("E27 status overall", r.status?.overall, "succeeded");
});
