...
Current release: **v2.2.0** · Last updated: 2026-09-21 · Proposal only — no committed dates.
...
|---|---|---|
| Shipped | v2.2.0 | Agent skills & MCP configuration |
...
|---|---|---|
| v2.2.0 | **Agent skills & MCP configuration** | Top-level `skills` / `mcpServers` registries in any pipeline config plus per-step `skills` / `mcp` refs, validated at load (12 die-rules; refs checked for the steps in THIS run). Before each run-order group launches, the flow materializes harness-idiomatic files into the worktree — claude: `.mcp.json` + `.claude/skills/<name>/SKILL.md`; cursor: `.cursor/mcp.json` + `.cursor/rules/<name>.mdc`; gemini: `.gemini/settings.json` (merged mcpServers) + a marked `GEMINI.md` section; opencode: `opencode.json` (`mcp` key) + a marked `AGENTS.md` section; codex: skills via `AGENTS.md` only, MCP unsupported (warn + skip). `${env:VAR}` expands inside mcp defs (unset → warn + empty string); parallel groups union their refs; unknown agents warn and get no files — never a killed run. Everything written is tracked in the `.orca-agent-config.json` manifest and restored after each group, with startup self-heal after crashes; pre-existing files are merged (JSON) or appended between `<!-- orca-agent-config BEGIN/END -->` markers (markdown) and restored verbatim; pre-existing skill folders are never overwritten (warn + skip). Pinned E2E by S1-S6 + F13-F17; suite 406/406 |
