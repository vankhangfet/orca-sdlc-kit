...
- **Worktree auto-create — start the flow from anywhere** *(v3.2.0)* — launching outside an Orca worktree no longer dead-ends: the flow offers to create a fresh `flow-<objective>-<timestamp>` worktree from your current HEAD (uncommitted changes stay behind) and runs the pipeline in it — diacritic-safe slugs, so Vietnamese objectives work out of the box. Interactive terminals get a `[y/N]` ask; CI/non-interactive opts in with `--create-worktree` or `"defaults": { "autoCreateWorktree": true }`. Pinned worktrees, `--dry-run`, and `--status-preview` are never touched, and the end-of-run summary prints the exact `orca worktree rm` cleanup command.
