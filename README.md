...
| Retry harder on failures | raise `"maxRetries"` (how often review/test failures loop back to coding); give ONE loop-heavy step its own budget with `"maxRetries"` on that step (e.g. planning loops 20x while review/test stay at 2) |
